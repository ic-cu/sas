...
public class RelazioniComplessi
{
  ...
  public RelazioniComplessi(Connection conn)
  {
    ...
    try
    {
      ...
      stmtDComparcPrimoLivello = conn.prepareStatement(comparcProp
          .getProperty("query.comparc.pl.full"));
      stmtDComparcSottoLivelli = conn.prepareStatement(
          comparcProp.getProperty("query.comparc.sl.full"),
  ...
  /*
   * Questo metodo estrae un iteratore di coppie (idComparc, codiProvenienza).
   * Esso comincia dal primo livello che è quello immediatamente legato ad un
   * istituto il cui id è il parametro; poi invoca un metodo ricorsivo che, per
   * ogni complesso, estrae i suoi figli fino ad arrivare alle foglie, sempre
   * creando coppie (idComparc, codiProvenienza) che saranno accodate
   * all'iteratore.
   */

  public Iterator<String[]> getComparcIterator(int idIstituto,
      String codiProvenienzaPadre)
  {
    ResultSet rs = null;
    ArrayList<String[]> al;
    ...
    try
    {
      ...
      while(rs.next())
      {
        ...
        log.info("Complesso " + idComplesso + ", codice provenienza "
            + codiProvenienza + ", figli " + numFigli);
  ...
  public ArrayList<String[]> getSubComparcList(String idComplesso,
      String codiProvenienzaPadre)
  {
    ArrayList<String[]> al;
    ArrayList<String[]> pl;
    ...
    try
    {
      ...
      while(rs.next())
      {
        ...
        /*
         * Aggiunge alla lista globale dei complessi questo figlio, ma se esso
         * stesso ha dei figli, lo aggiunge ad una lista locale di padri che
         * sarà scandita dopo la fine del result set
         */
        log.info("Complesso " + idFiglio + ", codice provenienza "
            + codiProvenienzaFiglio + ", figli " + numFigli);
        al.add(new String[] { idFiglio, codiProvenienzaPadre,
            codiProvenienzaFiglio, numFigli });
        if(Integer.parseInt(numFigli) > 0)
          pl.add(new String[] { idFiglio, codiProvenienzaFiglio, numFigli });
      }

      /*
       * Il result set è finito, quindi tutti i figli (non nipoti) di questo
       * complesso sono stati inclusi nell'elenco globale. Si scorre l'elenco di
       * quei figli che sono a loro volta padri e si effettua la ricorsione
       */
  ...
  public Iterator<Relazioni> createEntity(int idIstituto)
  {
     ...
     * fra cui il codi_provenienza necessario a costruire correttamente le
     * relazioni
     */
    ...
    {
      log.warn("Istituto " + siglaIstituto + " (" + idIstituto + ") scartato: "
          + e.getMessage());
    }
    ...
    while(idComplessi.hasNext())
    {
      ...
      numFigli = tripla[2];
      log.info("Istituto " + siglaIstituto + ", collego complesso "
...
// coppia si crea l'opportuna relazione e si aggiunge alla lista di relazioni.
      ...
      if(Integer.parseInt(numFigli) > 0)
      {
        idSubComplessi = getSubComparcList(idComplesso, codiProvenienzaComparc)
            .iterator();
        while(idSubComplessi.hasNext())
        {
          quadrupla = idSubComplessi.next();
          codiProvenienzaComparcSup = quadrupla[1];
          codiProvenienzaComparc = quadrupla[2];
          rel = relObjF.createRelazioni();
    ...
    /*
     * Finiti i complessi, si riporta al chiamante l'iteratore alla lista dei
     * complessi
     */
